...
文件没有 ICC 配置，72 DPI 来自显式 Image Resource 1005，不能把不存在的色彩配置报告为 sRGB。图层使用 `normal` 混合、不透明度 255、无蒙版和效果，不覆盖分组、文字、变换、复杂视觉依赖或性能规模。截断和损坏场景可以从基础文件在测试中构造，避免重复保存大量相似二进制。

## 格式依据

[Adobe Photoshop File Formats Specification](https://www.adobe.com/devnet-apps/photoshop/fileformatashtml/) 的以下部分：

- File Header：`8BPS`、version 1、RGB color mode 3、3 个合成通道及大端整数。
