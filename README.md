...
当前工程为 **M0 PSD 解析原型与 M1 最小多文档桌面预览**：桌面已接入文件选择、打开／切换／关闭／重载、作业取消、保存时合成预览及每份文档的缩放和平移。独立 Rust 核心统一管理文档修订、有界后台、LRU 缓存与资源计费，并提供 PSD 图层分类、文字原文和能力诊断。图层树、只读属性面板、完整素材服务和 MCP 属于后续开发范围。Windows release WebView 的真实 PSD 流程已验证，原生文件选择器点击及窗口退出仍待补充验收；这不代表 M1 全部完成。设计见 [docs](docs/README.md)，当前任务与验证记录见 [devlog](devlog/README.md)。
