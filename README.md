...
**M2-01 已实现核心选区、不可变快照与任务绑定**，包括范围规范化、版本校验、幂等释放及有界内容分页；尚未接入桌面多选／区域交互或协议接口，不能视为 M2 整体出口完成。产品化素材导出及 MCP 仍属后续范围。设计见 [docs](docs/README.md)，当前阶段与验证记录见 [devlog](devlog/README.md)。
...
快照数量、范围字节、旧选择缓存和任务记录均受独立额度约束，不提高 M1 源修订／预览额度；已释放任务仍保留有界幂等记录。内容页只计核心 JSON，不包含未来 IPC／MCP 信封。默认值、限制与未实现项见[当前核心选区与任务实现](docs/02-技术架构.md#当前核心选区与任务实现m2-01)；这组 Rust API 尚未成为桌面 DTO 或 MCP 接口。
