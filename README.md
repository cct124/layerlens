...
当前工程已完成 **M0 PSD 解析原型与 M1 多文档桌面查看器的有限范围验收**：桌面已接入文件选择、打开／切换／关闭／重载、作业取消、保存时合成预览、每份文档的缩放和平移，以及图层树、只读属性和画布定位。独立 Rust 核心统一管理文档修订、有界后台、LRU 缓存与资源计费，并提供 PSD 图层分类、分段文字样式和能力诊断。完整自动检查与公开样本核心回归通过；2026-09-20 用户完成 Windows 开发版桌面手测，反馈验收清单各项正常。此结论不代表完整 PSD 兼容、新一轮性能实测或安装发布验收。选区与任务快照、产品化素材导出及 MCP 属于后续开发范围。设计见 [docs](docs/README.md)，当前阶段与验证记录见 [devlog](devlog/README.md)。
...
同一租约提供只读图层检查：列表按页（单页 ≤128 条、响应 ≤256 KiB）读取，详情一次只取一个图层，文字按 UTF-16 分页（≤2048 code unit、每类 ≤128 段样式）续读且不拆分代理对。详情按完整 JSON ≤256 KiB 缩短文字页，保留原文绝对样式区间；单项属性或不可分页元数据超限时明确拒绝。`select_layer` 只在当前修订内提交选择，重载、关闭与旧修订的引用被拒绝；完整图层与文字不进入工作区通知，桌面只按 `nextOffset`／`textStart` 续读。
